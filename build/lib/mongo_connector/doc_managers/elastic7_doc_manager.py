...
try:
    ...
    raise ImportError("Error: elasticsearch 7.x is not installed.\n"
...
class BulkBuffer(object):
    ...
    def add_upsert(self, action, meta_action, doc_source, update_spec):
        ...
        if update_spec:
            ...
            self.add_doc_to_update(action, update_spec,
                                   len(self.action_buffer) - 2)
        else:
    ...
    def add_doc_to_update(self, action, update_spec, action_buffer_index):
        ...
        # LOG.error("get_from_ES is {}".format(get_from_ES))
        self.doc_to_update.append(
            (doc, update_spec, action_buffer_index, get_from_ES))
    ...
    def should_get_id(self, action):
        ...
        """
        # LOG.error("action is {}".format(action))
        # mapping_ids = self.doc_to_get.setdefault(action["_index"], {})
        mapping_ids = self.doc_to_get.setdefault(action["_index"],
                                                 {}).setdefault("_doc", set())
        # LOG.error("mapping_ids is {}".format(mapping_ids))
    ...
    def get_docs_sources_from_ES(self):
        ...
        docs = [
            doc for doc, _, _, get_from_ES in self.doc_to_update if get_from_ES
        ]
        # LOG.error("self.doc_to_update is {}".format(self.doc_to_update))
        if docs:
            documents = self.docman.elastic.mget(body={"docs": docs},
                                                 realtime=True)
            # LOG.error("docs is {} and documents {}".format(docs, documents))
    ...
    def update_sources(self):
        ...
        for doc, update_spec, action_buffer_index, get_from_ES in self.doc_to_update:
            if get_from_ES:
                ...
                # as it is up-to-date
                # source = self.get_from_sources(doc["_index"], doc["_type"],
                #                                doc["_id"])
            ...
            self.action_buffer[action_buffer_index][
                "_source"] = self.docman._formatter.format_document(updated)
